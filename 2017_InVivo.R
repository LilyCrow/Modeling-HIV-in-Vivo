...
plot_result <- ggplot(data = gathered_result, mapping=aes(x=time, y=value, color = variable)) + geom_line(linewidth=2) + theme_classic() + facet_wrap(~variable, scales = "free_y")

    #plot each equation on its own graph
    #subset(gathered_result, variable == "TR"), #plot declared equation(s) only
